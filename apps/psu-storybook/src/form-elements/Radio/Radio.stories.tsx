...
import { RadioItem, RadioGroup } from '@psu-flex/core-ui';
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <RadioGroup register={register} errors={errors} label="Radio Group Label">
        <RadioItem>A</RadioItem>
        <RadioItem>B</RadioItem>
      </RadioGroup>
    </form>
  );
};
const Template2 = () => {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      >
        <RadioItem checked>Yes</RadioItem>
...
const Template3 = () => {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      >
        <RadioItem>One</RadioItem>
        <RadioItem>Two</RadioItem>
      </RadioGroup>
    </form>
  );
};
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <Flex gap={4} direction="column">
          <RadioGroup
            ...
            label="Label"
          >
            <RadioItem>Dogs</RadioItem>
            <RadioItem>Cats</RadioItem>
          </RadioGroup>
          <CtaButton
            hocId={exampleHocId}
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <RadioGroup register={register} errors={errors} label="Label">
        <RadioItem disabled>Choice 1</RadioItem>
        <RadioItem>Choice 2</RadioItem>
      </RadioGroup>
    </form>
  );
};
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      <RadioGroup register={register} errors={errors} label="Label">
        {radioGroupData.map((item) => {
          return <RadioItem>{item.text}</RadioItem>;
        })}
      </RadioGroup>
    </form>
  );
